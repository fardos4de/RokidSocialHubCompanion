...
public final class SocialDb extends SQLiteOpenHelper {
    ...
    public static SocialDb get(Context context) {
        if (instance == null) {
            synchronized (SocialDb.class) {
                if (instance == null) {
                    instance = new SocialDb(context.getApplicationContext());
                }
            }
        }
        return instance;
    ...
    private SocialDb(Context context) {
    ...
    public synchronized void insertMessage(String source,
        ...
        values.put("conversation_title", conversationTitle);
        ...
        // Keep the phone-side cache bounded while retaining enough history for later glasses sync.
        getWritableDatabase().execSQL(
                "DELETE FROM messages WHERE conversation_id=? AND id NOT IN " +
                        "(SELECT id FROM messages WHERE conversation_id=? ORDER BY timestamp DESC LIMIT 100)",
    ...
    public synchronized List<Models.Conversation> recentConversations(int limit) {
        ...
        String sql = "SELECT m.source,m.conversation_id,m.conversation_title,m.body,m.timestamp,m.notification_key " +
                "FROM messages m JOIN (SELECT conversation_id,MAX(timestamp) max_time FROM messages GROUP BY conversation_id) x " +
                "ON m.conversation_id=x.conversation_id AND m.timestamp=x.max_time " +
                "GROUP BY m.conversation_id ORDER BY m.timestamp DESC LIMIT ?";
        try (Cursor cursor = getReadableDatabase().rawQuery(sql, new String[]{String.valueOf(limit)})) {
            while (cursor.moveToNext()) {
                ...
                c.source = cursor.getString(0);
                c.conversationId = cursor.getString(1);
                c.title = cursor.getString(2);
                c.lastBody = cursor.getString(3);
                c.lastTimestamp = cursor.getLong(4);
                c.latestNotificationKey = cursor.getString(5);
                result.add(c);
            }
        }
        return result;
    ...
    public synchronized List<Models.Message> recentMessages(String conversationId, int limit) {
        ...
        String sql = "SELECT id,source,conversation_id,conversation_title,sender,body,timestamp,notification_key,outgoing " +
        ...
        try (Cursor cursor = getReadableDatabase().rawQuery(sql, new String[]{conversationId, String.valueOf(limit)})) {
            while (cursor.moveToNext()) {
                ...
                m.source = cursor.getString(1);
                m.conversationId = cursor.getString(2);
                m.conversationTitle = cursor.getString(3);
                m.sender = cursor.getString(4);
                m.body = cursor.getString(5);
                m.timestamp = cursor.getLong(6);
                m.notificationKey = cursor.getString(7);
                m.outgoing = cursor.getInt(8) != 0;
                result.add(m);
            }
        }
        return result;
    ...
    public synchronized long enqueueText(String source,
                                         String conversationId,
                                         String conversationTitle,
                                         String notificationKey,
                                         String text,
    ...
    public synchronized List<Models.OutboxItem> pendingOutbox() {
        ...
        try (Cursor cursor = getReadableDatabase().rawQuery(
                "SELECT id,source,conversation_id,conversation_title,notification_key,type,payload,state,created_at,last_error " +
                        "FROM outbox WHERE state IN ('QUEUED','RETRY') ORDER BY created_at ASC", null)) {
            while (cursor.moveToNext()) {
                Models.OutboxItem item = new Models.OutboxItem();
                item.id = cursor.getLong(0);
                item.source = cursor.getString(1);
                item.conversationId = cursor.getString(2);
                item.conversationTitle = cursor.getString(3);
                item.notificationKey = cursor.getString(4);
                item.type = cursor.getString(5);
                item.payload = cursor.getString(6);
                item.state = cursor.getString(7);
                item.createdAt = cursor.getLong(8);
                item.lastError = cursor.getString(9);
                result.add(item);
            }
        }
        return result;
    ...
    public synchronized int outboxCount() {
        try (Cursor cursor = getReadableDatabase().rawQuery(
                "SELECT COUNT(*) FROM outbox WHERE state IN ('QUEUED','RETRY')", null)) {
